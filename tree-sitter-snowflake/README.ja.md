...
- 各トップレベルステートメント（`；` までのトークンの連続）は、先頭キーワードに基づく粗い文種別ノード（`select_statement`、`insert_statement`、`create_statement` など。その他はすべて寛容な `statement` フォールバック）にグループ化され、折りたたみ・アウトライン・ナビゲーションに使えます；
