...
VS Code 拡張機能は、Snowflake SQL 言語メタデータ、TextMate 文法ファイル、およびローカルフォーマッタを提供します。ユーザーが Format Document、Format Selection、または保存時フォーマットを実行すると、SQL は拡張機能にバンドルされた WebAssembly モジュールによってローカルでフォーマットされ、エディタに書き戻されます。テレメトリ、分析、ネットワークアップロード、またはリモートフォーマットは含まれておらず、SQL テキストを保存することもありません。
