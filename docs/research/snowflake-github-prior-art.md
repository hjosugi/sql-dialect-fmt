...
- `sql-formatter-org/sql-formatter` — JavaScript pretty-printer with Snowflake support, but explicitly does not support stored procedures. Useful as a formatter UX reference and a warning that Snowflake Scripting/procedures are a core differentiator for snow-fmt. Source: https://github.com/sql-formatter-org/sql-formatter
...
- SQLFlex (SIGMOD/PACMMOD 2026) argues that dialect-specific syntax is a major
  ...
  dialect fragments behind validated segmentation. snow-fmt should keep the core
...
## Design Implications For snow-fmt

- Keep the lexer lossless and permissive. SQLFluff/tree-sitter-sql both show the value of surviving dialect edges; snow-fmt should continue returning tokens/ranges even when grammar support lags.
...
- Highlighting should start lexical and become semantic. The new `snow-fmt-highlight` crate gives stable ranges/scopes now; later LSP semantic tokens can layer parser context on top without replacing it.
