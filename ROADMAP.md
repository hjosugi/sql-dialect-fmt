...
**残りの主な未着手（価値順）**:
1. **Phase 8 埋め込み言語**: `$$…$$` の言語判定→サブフォーマッタ委譲（JS=biome、SQL=自己再帰、Python 方針決定）。トップレベル Snowflake Scripting は構造化済み、procedure body は現在 verbatim 保持で無破壊。
