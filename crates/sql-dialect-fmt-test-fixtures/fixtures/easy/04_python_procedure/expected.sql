...
def main(
    ...
    if not re.fullmatch(r"[A-Za-z0-9_.$\"]+", p_source_table):
        raise ValueError("Unsafe source table identifier")
    if not re.fullmatch(r"[A-Za-z0-9_$\"]+", p_text_column):
        raise ValueError("Unsafe text column identifier")

    rows = (
        session.table(p_source_table)
        .select(col(p_text_column).cast("string").alias("TEXT_VALUE"))
    ...
    for row in rows:
        original = safe_text(row["TEXT_VALUE"])
        ...
        for script in scripts or ["latin_or_other"]:
            ...
            examples.append(
                {
                    ...
                    "scripts": scripts,
                    "code_points": [f"U+{ord(ch):04X}" for ch in original[:24]],
                }
            )
    ...
    return {
        ...
        "examples": examples,
        "messages": {
            "ja": "Unicodeプロファイルが完了しました。",
            "ko": "유니코드 프로파일링이 완료되었습니다.",
            "ar": "اكتمل تحليل يونيكود.",
            "hi": "यूनिकोड प्रोफ़ाइल पूरी हुई।",
        },
    }
$$;
