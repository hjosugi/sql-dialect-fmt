//! `snow-fmt.toml` configuration files.
//!
...
//! `snow-fmt.toml` — the first one found on the way up — mirroring how `rustfmt`, `prettier`, and
...
/// A parsed `snow-fmt.toml`. Every field is optional; absent fields fall back to the formatter
...
/// Find the nearest `snow-fmt.toml` at or above `start`, returning its path (not its contents).
///
